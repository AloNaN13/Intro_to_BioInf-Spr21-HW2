...
# hClust
hc.result <- hclust(data.dist, "ward.D")
# DBSCAN
...
Note: as opposed to different methods to set our parameters we found along the way, we decided that we would like to try the exploratory approach - meaning we tried to find the best parameters for each clustering method by looking at our data mainly visually.
...
This time, the plots shows us that the number of clusters that will give us with the most accurate clustering is 6, while the parameters are: epsilon = 1, minPts = 6. Clustering using these parameters provides us with clusters that seems to be as far apart from each other, while also having the points in eacch cluster closest to each other.

# Q3.3
The method in hclust() that seems to give a clustering that is more accurate to the tSNE plot is" "ward.D2", using k=7 ("cutting" the dendrogram into 7 different clusters). The dendrogram using these parameters will look as following:
...
rect.hclust(hc.result, k = best_k, border = 2:5)
...
As can be seen in the tables, it seems that **RACE** and **GENDER** do not expalin the clusters that were chosen by the algorithm-
*GENDER* - expect for **cluster number 2 **, the number of samples in each cluster, divided into "Male" and "Female", is almost identical. Even in cluster number 2 the difference doesn't seems as significant.
