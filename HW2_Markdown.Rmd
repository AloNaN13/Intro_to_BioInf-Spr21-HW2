...
# hClust
method_values <- list("complete","ave","ward.D","ward.D2","centroid")
for(m in method_values){
  hc.result <- hclust(data.dist, method=m)
  plot(hc.result, cex = 0.6, hang = -1)
  plot(tsne$Y, col=hc.result$order, main = paste( "Hierarchical clustering: method=",m))
}
...
hc.res <- hclust(data.dist, "ward.D")
...
hc.result <- hclust(data.dist, "complete")
# DBSCAN
dbscan.result <- dbscan(tsne$Y, eps = 1, minPts = 4)
